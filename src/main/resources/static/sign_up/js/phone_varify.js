...
function phone_varify_send() {
    ...
    if (re.test(tel)) {
        $.ajax({
            url: recruit_uri+"/sms/sendCode" + '?mobile=' + mobile.value,
            type: "post",
            dataType: "json",
...
function phone_varify_code() {
    ...
    if (code_usr.length == 4) {
        $.ajax({
            url: recruit_uri+"/sms/verifyCode" + "?mobile=" + mobile.value + "&code=" + code_usr,
            type: "post",
            dataType: "json",
            success: function (msg) {
                if (msg == 1) {
                    ...
                    $.ajax({
                        url: recruit_uri+"/user/registerPost" + "?mobile=" + mobile.value + "&password=" + pass + "&nickName=" + nickName,
                        type: "post",
                        dataType: "json",
                        success: function (output) {
                            ...
                            if (output == "1") {
                                ...
                                setTimeout(function () {
                                    window.location.href = recruit_uri+"/user/login";
                                }, 3000);
